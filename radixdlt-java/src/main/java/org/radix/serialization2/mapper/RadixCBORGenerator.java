...
public class RadixCBORGenerator extends GeneratorBase
{
    ...
    private final static long MIN_INT_AS_LONG = Integer.MIN_VALUE;
    private final static long MAX_INT_AS_LONG = Integer.MAX_VALUE;
    ...
    public RadixCBORGenerator(IOContext ctxt, int stdFeatures, int formatFeatures,
    ...
    public RadixCBORGenerator enable(Feature f) {
    ...
    public RadixCBORGenerator disable(Feature f) {
        _formatFeatures &= ~f.getMask();
        if (f == Feature.WRITE_MINIMAL_INTS) {
    ...
    public RadixCBORGenerator configure(Feature f, boolean state) {
        if (state) {
            enable(f);
        } else {
            disable(f);
            ...
            throws IOException
    {
        ...
        writeLongString(text, offset, len);
    }
